                        ...
                        'Save': function(){
                            $.ajax({ 
                                ...
                                data: { 
                                    ...
                                    'status[description]': $("#status-description").val(),
                                    'status[image]': $("input[name=status-image]:checked").val(),
                                    'status[level]': $("#statusLevel").val()
                                },
                                dataType: 'json', 
                                context: tr,
...
stashboard.fillIndex = function() {
    ...
    var createServiceRow = function(data, fetchStatuses){
        ...
        if (fetchStatuses){
            ...
            $.ajax({ 
                type: "GET",
                url: "/api/v1/services/" + data.id + "/events/current.json",
                dataType: 'json', 
	    ...
	    rightNow.setSeconds(0);
            ...
            var url = "/api/v1/services/" + data.id + "/events.json";
...
stashboard.fillService = function(serviceName, isAdmin, start_date, end_date) {
    ...
    $.ajax({
        type: "GET",
        url: "/api/v1/services/" + serviceName + '.json',
        dataType: "json",
        success: function(service){
            ...
            eventsURL = "/api/v1/services/" + service.id + "/events.json";
                ...
                $("#delete-service-modal").dialog({
                    ...
                    buttons: {
                        'Delete Service': function(){
                            $.ajax({ 
                                type: "DELETE",
                                url: "/api/v1/services/" + service.id + '.json',
                                dataType: 'json', 
                                success: function(data){
                                    location.replace("/");  
                                }
                ...
                $("#add-event-modal").dialog({
                    ...
                    buttons: {
                        'Update Status': function(){
                            $.ajax({ 
                                type: "POST",
                                url: "/api/v1/services/" + service.id + "/events.json",
                                data: {
                                    'event[status]': $("#statusValue").val(),
                                    'event[message]': $("#eventMessage").val()
                                },
                                dataType: "json",
                    ...
                    buttons: {
                        'Add Note': function(){
                            $.ajax({ 
                                ...
                                url: "/api/v1/services/" + service.id + "/events.json",
                                data: {
                                    'event[message]': $("#noteMessage").val(),
                                    'event[is_informational]': "true"
                                },
                                dataType: "json",
